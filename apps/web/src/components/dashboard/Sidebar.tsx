    ...
    <div className="flex h-full flex-col">
      ...
      >
        {navSections.map((section) => (
          <div key={section.id}>
            <p
              className={`mb-2 px-2.5 text-[10px] font-semibold uppercase tracking-[0.12em] ${text.faint}`}
            >
              {section.label}
            </p>
        ...
        <div className={`mt-auto shrink-0 border-t ${borders.subtle} pt-4`}>
          ...
          <ul>
            <li>
              <SidebarNavItem item={footerNavItem} onNavigate={onNavigate} />
            </li>
          </ul>
        </div>
      </nav>
  ...
  return (
    <>
      ...
      {mobileOpen ? (
        <button
          type="button"
          className="fixed inset-0 z-40 bg-slate-100 backdrop-blur-sm lg:hidden"
      ...
      <aside
        className={[
          `fixed top-0 left-0 z-50 h-screen w-64 ${surfaces.sidebar} shadow-sm`,
