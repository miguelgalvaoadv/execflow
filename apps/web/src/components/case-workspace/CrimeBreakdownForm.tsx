...
export function CrimeBreakdownForm({ crimes, onChange }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {!isAdding && (
          <button
            ...
            className="text-[11px] bg-zinc-800 hover:bg-zinc-700 text-white font-medium py-1 px-2.5 rounded border border-zinc-700 transition-colors"
          >
            + Adicionar Crime
          </button>
        )}
      ...
      {crimes.length > 0 && (
        <ul className="space-y-2">
          {crimes.map((c) => (
            <li
              key={c.id}
              className="flex flex-col bg-black/40 border border-zinc-800 p-2.5 rounded text-[11px]"
            >
              ...
              <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-zinc-800/60">
                <div className="text-zinc-300">
                  <span className="text-zinc-500">Pena:</span> {c.sentenceDays} dias
                </div>
                <div className="flex gap-2">
                  {c.isHediondo && (
                    <span className="text-[9px] bg-red-950/40 border border-red-900/60 text-red-400 px-1 rounded uppercase">
                      Hediondo
                    </span>
                  )}
                  {c.isEquiparado && (
                    <span className="text-[9px] bg-orange-950/40 border border-orange-900/60 text-orange-400 px-1 rounded uppercase">
                      Equiparado
                    </span>
                  )}
                  {c.hasResultingDeath && (
                    <span className="text-[9px] bg-purple-950/40 border border-purple-900/60 text-purple-400 px-1 rounded uppercase">
                      Morte
                    </span>
                  )}
                </div>
                <div className="text-zinc-400">
                  <span className="text-zinc-500">Sentença:</span> {new Date(c.sentenceDate).toLocaleDateString('pt-BR')}
                </div>
                <div className="text-zinc-400">
                  <span className="text-zinc-500">Trânsito:</span> {new Date(c.transitDate).toLocaleDateString('pt-BR')}
                </div>
              </div>
            </li>
          ))}
      ...
      {isAdding && (
        <div className="bg-zinc-900/50 border border-zinc-700/50 p-3 rounded space-y-3">
          <h5 className="text-[11px] font-semibold text-white">Novo Crime</h5>
          
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Código Único *</label>
              <input
                type="text"
                placeholder="Ex: ROUBO_MAJORADO"
                value={newItem.crimeCode || ''}
                onChange={(e) => setNewItem({ ...newItem, crimeCode: e.target.value.toUpperCase() })}
                className="w-full bg-black/60 border border-zinc-800 rounded px-2 py-1.5 text-[11px] text-white focus:outline-none focus:border-indigo-500 uppercase"
              />
            </div>
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Nome do Crime *</label>
              <input
                type="text"
                placeholder="Ex: Roubo Majorado"
                value={newItem.crimeName || ''}
                onChange={(e) => setNewItem({ ...newItem, crimeName: e.target.value })}
                className="w-full bg-black/60 border border-zinc-800 rounded px-2 py-1.5 text-[11px] text-white focus:outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Artigo *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Lei *</label>
              <input
                type="text"
                placeholder="Ex: CP"
                value={newItem.law || ''}
            ...
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Pena (dias) *</label>
              <input
                type="number"
                min={1}
                placeholder="Ex: 1950"
            ...
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Data da Sentença *</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-[10px] text-zinc-400 mb-1">Data do Trânsito *</label>
              <input
                type="date"
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-zinc-800">
            ...
            <button
              ...
              className="text-[10px] bg-indigo-600 hover:bg-indigo-500 text-white font-medium px-3.5 py-1.5 rounded transition-colors"
            >
              Adicionar à Lista
            </button>
          </div>
