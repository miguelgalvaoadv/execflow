...
export function EditCaseModal({ open, onClose, caseData }: EditCaseModalProps) {
  ...
  const [internalRef, setInternalRef] = useState(caseData.internalRef)
  const [executionProcessNumber, setExecutionProcessNumber] = useState(caseData.executionProcessNumber || '')
  const [originProcessNumber, setOriginProcessNumber] = useState(caseData.originProcessNumber || '')
  const [courtName, setCourtName] = useState(caseData.courtName || '')
  const [courtJurisdiction, setCourtJurisdiction] = useState(caseData.courtJurisdiction || '')
  const [caseKind, setCaseKind] = useState<string>(caseData.caseKind || 'primary')
  const [sentenceSummary, setSentenceSummary] = useState(caseData.sentenceSummary || '')

  useEffect(() => {
    if (open) {
      setInternalRef(caseData.internalRef)
      setExecutionProcessNumber(caseData.executionProcessNumber || '')
      setOriginProcessNumber(caseData.originProcessNumber || '')
      setCourtName(caseData.courtName || '')
      setCourtJurisdiction(caseData.courtJurisdiction || '')
      setCaseKind(caseData.caseKind || 'primary')
      setSentenceSummary(caseData.sentenceSummary || '')
    }
  }, [caseData, open])

  const mutation = useMutation({
    mutationFn: (data: UpdateCaseInput) =>
      apiPatch<{ data: unknown }>(`/api/v1/cases/${caseData.id}`, data, {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    const body: UpdateCaseInput = {
      ...
      ...(caseKind ? { caseKind: caseKind as UpdateCaseInput['caseKind'] } : {}),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <h2 className={`text-[20px] font-semibold tracking-[-0.01em] mb-1 ${text.primary}`}>
          Editar Caso de Execução Penal
        </h2>
        <p className={`text-[13px] mb-6 ${text.muted}`}>
          Atualize as configurações do processo de execução penal.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              type="submit"
              variant="primary"
              disabled={mutation.isPending || internalRef.trim() === ''}
            >
